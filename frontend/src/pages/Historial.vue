	  ...
	  <div class="h-screen flex bg-color backdrop-blur-2xl pr-3 pl-3">
		...
		<div class="flex-initial w-2/3 pt-6 pl-6 h-full h-min-screen" v-if="lessons.length>0">
		  <div class="grid grid-cols-1 gap-1">
				<router-link   v-for="lesson in lessons" v-bind:to="{name: 'Lesson', params:{lesson:lesson.name} }" class="bg-card rounded-md drop-shadow-lg p-4 shadow-sm text-sm">
				  {{lesson.title}}
			  </router-link>
		  </div>
		</div>
		...
		if(this.band2 == 0){
		  ...
		  frappe.call({
			...
			args: {
					  ...
					  "filters":[["capacitacion","=",lesson]],
					  "limit":100
			},
		  async:false,
			callback(res) {
			  prt.band2=0
			  return prt.lessons = res.message;
			}
		  });
		}
	  }
